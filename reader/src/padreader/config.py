...
class ChromaConfig:
    ...
    **현장 실물이 바뀌면 이 값을 바꾼다.** 2026-08-28 촬영분에서 실물이 개정
    도안으로 교체된 것을 확인하고 ``v3``(정사각 측정면, 좌우 흰 여백, 선군
    없음)로 올렸다. 교체 전 도안은 ``v2``(측정면 1.40:1, 선군 있음)다.

    실물 교체는 하루에 끝나지 않아 두 세대가 한동안 섞인다. 그래서 이 값
    하나로 못 박지 않고 ``fallback_specs`` 를 함께 시도한다.
    ...
    fallback_specs: list[str] = field(default_factory=lambda: ["v2"])
    """``spec`` 으로 읽히지 않을 때 이어서 시도할 유채색 규격.

    도안 세대가 섞인 기간을 위한 것이다. 패드를 한 번에 다 바꿀 수는 없으므로
    같은 순회에 신·구 도안이 함께 찍힌다. 규격을 하나로 못 박으면 그중 한쪽이
    통째로 판독되지 않는다 - 실제로 개정 규격으로 올린 뒤 교체 전 촬영분의
    유채색 다섯 건이 전부 실패했다.

    어느 쪽인지는 **재 보고 고른다.** 규격마다 검출부터 앵커 정규화까지 돌려
    보고 흑·백 앵커 대비가 실제로 성립하는 쪽을 쓴다. 규격이 틀리면 앵커 창이
    패치를 벗어나 대비가 0 이나 음수가 되므로 그 자체로 갈린다 - 임계값을
    새로 들이지 않는다.
