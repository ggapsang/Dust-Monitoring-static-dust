...
도안 생성기(``tools/generate_pad.py``)와 POINT_ID 판독기(``point_id.py``)가
...
산세리프로 인쇄되어 있다. 그 도안으로 찍은 사진은 POINT_ID 판독이 틀릴 수
있으므로, 실제 인쇄에 쓴 폰트를 ``point_id.font_dir`` 폴더에 넣어 둔다.
"""
