...
패드에 인쇄된 번호는 **패드가 붙어 있는 물리적 관측 개소**를 가리킨다. AMR
쪽 ``TARGET_ID`` 와 같은 것이 아니다 — 그쪽은 한 웨이포인트에서 PTZ 프리셋을
잡고 찍은 **사진 한 장**의 식별자다. 한 장에 패드가 여러 개 찍히므로 촬영
단위와 개소는 1:N 이고, 같은 이름을 쓰면 상위 계층에서 서로 다른 것이 같은
필드로 만난다.
...
전제: 인쇄에 쓴 폰트와 판독에 쓰는 폰트가 같아야 한다. ``point_id.font_dir``
...
def read_point_id(
    ...
    cfg: PointIdConfig,
    pad_size_px: int,
) -> tuple[str | None, PointIdStatus, float | None]:
    """(읽은 값, 상태, 신뢰도)."""
    if not cfg.enabled:
        return None, PointIdStatus.DISABLED, None

    patch = crop(rectified_gray, spec.point_id_box, pad_size_px)
    if patch.size == 0:
        return None, PointIdStatus.FAILED, None
    ...
    if best is None:
        return None, PointIdStatus.FAILED, None
    return best[2], PointIdStatus.OK, best[1]
