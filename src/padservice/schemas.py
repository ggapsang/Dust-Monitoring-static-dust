            ...
            "examples": [
                {
                    ...
                    "pads": [
                        {
                            "success": True,
                            "summary": "판독 성공 · combined 0.307 (uniform 0.036 · localized 0.305) · target_id 1078",
                            "target_id": "1078",
...
def pad_summary(pad: dict[str, Any]) -> str:
    ...
    if pad.get("target_id"):
        parts.append(f"target_id {pad['target_id']}")
    return " · ".join(parts)
...
def build_summary(payload: dict[str, Any]) -> str:
    ...
    parts = []
    for pad in pads:
        name = pad.get("target_id") or "번호 미상"
        if pad.get("success"):
