...
# TARGET_ID 숫자가 놓이는 영역. 실측 x 431..775, y 112..211 을 감싸도록
...
# 측정 여백. 인쇄물이 상단 밴드(모서리 블록 TL/TR, TARGET_ID)와 하단
# 밴드(선군, 모서리 블록 BL)에만 있으므로, 그 사이의 중간 밴드 전체가
# 인쇄물 없는 영역이다. 여기를 격자로 나누면 마스킹이 필요 없고 모든
# 구획의 유효 면적이 같아진다.
#
# 캔버스 기준 인쇄물 하한은 y=211(TARGET_ID), 상한은 y=940(선군 1단)이므로
...
#
# 상단 밴드의 빈 공간(모서리 블록과 TARGET_ID 사이)에 좌우 대칭으로 두 쌍을
...
class PadSpec:
    ...
    target_id_box: Rect = TARGET_ID_BOX
    ...
    def print_element_rects(self) -> dict[str, Rect]:
        ...
        out["target_id"] = self.target_id_box
