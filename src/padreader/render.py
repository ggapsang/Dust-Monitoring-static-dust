...
def render_pad(
    ...
    font_path
        TARGET_ID 에 쓸 TTF. ``None`` 이면 OpenCV Hershey 로 떨어진다.
    ...
    # TARGET_ID
    if target_id:
        glyph_h = max(8, int(round(spec.target_id_box.height * pad_px)))
        canvas.paste(render_text(target_id, glyph_h, font_path), spec.target_id_box, ink)
