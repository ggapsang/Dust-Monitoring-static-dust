...
def run(tone: str, params: CaptureParams, **overrides):
    image, _ = synthesize(SPEC, tone, params)
    merged = {"spec": SPEC_NAME}
    merged.update(overrides)
    return read_pad(image, tone, overrides=merged)
...
def test_overexposure_is_visible_in_quality(tone: str) -> None:
    ...
    gated = run(tone, blown, quality={"max_saturated_bright_ratio": 0.001})
...
def test_target_id_is_read(tone: str) -> None:
    for target in ("1078", "42", "9310"):
        ...
        result = read_pad(image, tone, overrides={"spec": SPEC_NAME})
...
def test_is_deterministic(tone: str) -> None:
    ...
    first = read_pad(image, tone, overrides={"spec": SPEC_NAME})
    second = read_pad(image, tone, overrides={"spec": SPEC_NAME})
...
def test_quality_gate_rejects_blurred_image(tone: str) -> None:
    ...
    measured = run(tone, blurred_params)
    limit = sharp.quality.edge_rise_ratio * 2.0

    gated = run(tone, blurred_params, quality={"max_edge_rise_ratio": limit})
...
def test_visualization_is_optional(tone: str) -> None:
    ...
    plain = read_pad(image, tone, overrides={"spec": SPEC_NAME})
    assert plain.rectified is None and plain.overlay is None

    drawn = read_pad(image, tone, overrides={"spec": SPEC_NAME}, visualize=True)
...
def test_black_level_sensitivity_by_reference(tone: str) -> None:
    ...
    for b in black_levels:
        ...
        result = read_pad(image, tone, overrides={"spec": "v2"})
        assert result.success, result.failure_detail
