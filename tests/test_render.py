...
도안과 같은 기하를 갖는지 확인한다. 폰트는 재현할 수 없으므로 TARGET_ID
모양은 비교하지 않는다.
"""
...
def gray(spec_obj: spec.PadSpec, tone: str, target_id: str = "1078") -> np.ndarray:
    img = render_pad(spec_obj, tone, target_id=target_id, pad_px=PAD_PX, channels=1)
    return img
...
def test_anchors_do_not_overlap_other_elements() -> None:
    ...
    s = spec.V2
    others = [s.target_id_box, *s.corner_blocks.values()]
...
def test_target_id_stays_inside_its_box(tone: str) -> None:
    """긴 ID 를 넣어도 박스를 넘지 않는지. 넘으면 여백을 오염시킨다."""
    background, _ = tone_levels(tone)
    for target in ("1", "1078", "999999"):
        img = render_pad(spec.V2, tone, target_id=target, pad_px=PAD_PX, channels=1)
